...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.goal_node is not None:
            reward+=self.goal_node.goal.reward(observation['achieved_goal'], self.last_state)
        ...
        self.last_state = observation['achieved_goal']
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.start_states is None:
            ...
            _, info = self.panda_env.reset(seed=seed, options=options)
            observation, state_id = None, None
            # while True:
            observation, state_id = random.sample(self.start_states, 1)[0]
                # if self.start_node.goal.predicate(observation['achieved_goal']):
                    # break
    ...
    def set_abstract_states(self, start_states: list, start_node, goal_node, avoid = None):
