...
def train_policy(env: gym.Env, start_node, end_node, n_episodes, stored_states: List = None):
    ...
    for episode in episodes:
        ...
        episodes.set_description(f"Current reach: {sum(reach)/len(reach):.2f}, total_reach: {sum(reach)}, reward: {statistics.mean(rewards):.2f}±{statistics.stdev(rewards):.1f}, ep_len: {statistics.mean(ep_lens):.2f}")
...
def test_policy( policy: PPO, env: gym.Env, start_node, end_node, n_episodes_test, stored_states: List = None):
    ...
    for episode in episodes:
        ...
        reach.append(reached or info['is_success'])
        rewards.append(reward)
        cached_states.insert(end_node.goal.current_goal, reached or info['is_success'])
        state_id = env.save_state()
