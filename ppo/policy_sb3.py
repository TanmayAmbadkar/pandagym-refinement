...
def sample_policy(env: gym.Env, observation:np.ndarray, policy:PPO):
    ...
    return final_terminated, total_reward, observation, traj


def train_policy(env: gym.Env, parent, end_node, n_episodes=3000, stored_states: list = None):


    # env.set_abstract_states(start_node, end_node)
    env.set_abstract_states(stored_states, parent, end_node)
...
def test_policy( policy: PPO, env: gym.Env, parent, end_node, n_episodes_test, stored_states: list = None):
    ...
    for episode in episodes:
        ...
        cached_states.insert(start_observation['achieved_goal'], reached)
